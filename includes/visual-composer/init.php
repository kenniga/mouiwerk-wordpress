	...
	if ( class_exists( 'WPBakeryVisualComposerAbstract' ) ) {
        ...
        function nm_vc_frontend_assets() {
            ...
            if ( ! $nm_globals['vcomp_stock'] ) {
                ...
                wp_enqueue_style( 'nm_js_composer_front', NM_THEME_URI . '/css/visual-composer/nm-js_composer.css', array(), NM_THEME_VERSION, 'all' );

                // Enqueue frontend scripts (original file with unused scripts removed)
                wp_enqueue_script( 'nm_composer_front_js', NM_THEME_URI . '/js/visual-composer/nm-js_composer_front.min.js', array( 'jquery' ), NM_THEME_VERSION, true );
            }
		...
		if ( ! $nm_globals['vcomp_stock'] ) {
            ...
            function nm_vc_tta_resources() {
                if ( wp_script_is( 'vc_accordion_script' ) || wp_script_is( 'vc_tabs_script' ) ) { // Make sure the stock "tta" scripts are enqueued
                    ...
                    // Enqueue scripts
                    wp_enqueue_script( 'nm_accordions_tabs', NM_THEME_URI . '/js/visual-composer/nm-accordions-tabs.min.js', array( 'jquery' ), NM_THEME_VERSION, true );
                }
            }
